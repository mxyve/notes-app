...
const MyLikeNotes = ({ children }) => {
  ...
  const fetchLikeNotes = async () => {
    try {
      setLoading(true);
      setError(null);
      const fetchNotes = await getLikeNote(user.id);
      console.log('fetchNotes', fetchNotes.data);
      setNotes(fetchNotes.data);
      //   return fetchNotes.data.length; // 返回点赞笔记数量
    } catch (error) {
      ...
      <Content className="p-4 max-w-5xl mx-auto">
        <div className="mb-6">
          <Title level={3} className="flex items-center">
            <LikeOutlined className="mr-2 text-red-500" />
            我点赞的笔记
          </Title>
          <Text type="secondary">共 {notes.length} 篇点赞笔记</Text>
        </div>
              ...
              renderItem={(note) => (
                ...
                >
                  ...
                  <div className="flex items-center mb-3">
                    <img
                      src={note.avatar_url || <UserOutlined />}
                      alt="用户头像"
